...
class StaticMethodVariable(VariableTracker):
    ...
    def __repr__(self) -> str:
        if isinstance(self.descriptor, BaseUserFunctionVariable):
            return f"StaticMethodVariable({self.descriptor.get_function().__name__})"
...
class ClassMethodVariable(VariableTracker):
    ...
    def __repr__(self) -> str:
        if isinstance(self.descriptor, BaseUserFunctionVariable):
            return f"ClassMethodVariable({self.descriptor.get_function().__name__})"
    ...
    def tp_descr_get_impl(
        ...
        bound_source = (
            AttrSource(owner.source, func.get_function().__name__)
            if self.from_class_attr and owner.source
            else None
        )
        if func.source is None and bound_source is not None:
            # from_descriptor had no source to derive the function source from,
            # so take it from the bound attribute. Without this the wrapped
            # function would go unguarded.
            func = UserFunctionVariable(
                func.get_function(), source=AttrSource(bound_source, "__func__")
            )
