...
class ListReverseIteratorVariable(BaseListIteratorVariable):
    ...
    def as_python_constant(self) -> Any:
        if self.index > 0 or self.is_exhausted:
            raise NotImplementedError
        # self.items is in iteration order; reversed() takes the reversed backing sequence
        # to return a list_reverseiterator rather than a list_iterator for type fidelity.
