...
TORCH_NCCL_NAN_CHECK). Receive buffers are not checked. A NaN raises a
RuntimeError; on an accelerator the check synchronizes to report it. The
process group owns the hook, so the returned handle only has to be kept if
